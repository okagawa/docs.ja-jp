...
|`null`|いずれも許可されません。|リテラルの `null`。 JSON から XML へのマッピングでは、は `null` xml にマップされない空白 (セクション2の ' ws ') で囲むことができます。<br /><br /> 例 : 次の要素は JSON フラグメントにマップされます。<br /><br /> `<root type="null"/>`<br /><br /> or<br /><br /> `<root type="null"></root>`<br /><br /> :<br /><br /> いずれの場合も、JSON フラグメントは `Null` です。|
