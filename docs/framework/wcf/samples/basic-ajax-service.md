...
<xref:System.ServiceModel.Description.WebScriptEndpoint> は、サービスの既定のデータ形式を XML ではなく JSON に設定します。 サービスを呼び出すには、 `http://localhost/ServiceModelSamples/service.svc/Add?n1=100&n2=200` このトピックで後述する「セットアップとビルドのセットアップ」の手順を完了した後に、に移動します。 このテスト機能は、HTTP GET 要求を使用することによって有効になります。
