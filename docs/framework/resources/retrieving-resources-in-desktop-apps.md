...
description: デスクトップ アプリでリソースを取得します。 メイン アセンブリを使用して既定の (ニュートラル) カルチャのリソースをパッケージ化し、各カルチャのサテライト アセンブリを作成します。
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
> [!NOTE]
> ASP.NET アプリを展開する場合には、スタンドアロン .resources ファイルを使用しないでください。 このファイルを使用すると、ロックの問題が生じ、XCOPY 展開が破損する恐れがあります。 ASP.NET リソースの展開先はサテライト アセンブリにすることをお勧めします。 詳細については、「 [ASP.NET Web Page Resources Overview](/previous-versions/aspnet/ms227427(v=vs.100))」を参照してください。  
...
- [Windows ストア アプリでのリソースの作成と取得](/previous-versions/windows/apps/hh694557(v=vs.140))
