...
```
 ...
 完全な追跡参加要素は、ConsoleTrackingParticipant.cs ファイルに実装されます。 カスタム追跡参加要素のメソッドを次のコード例に示し <xref:System.Activities.Tracking.TrackingParticipant.Track%2A> ます。
...
- は <xref:System.Activities.Tracking.CustomTrackingRecord> 、の track メソッドを呼び出すことによって出力され <xref:System.Activities.ActivityContext> ます。
