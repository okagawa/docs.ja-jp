...
> [!NOTE]
> コードアクセスセキュリティは非推奨とされるため、.NET エコシステム内の最新のコードのほとんどは、部分信頼環境で動作するように設計されていません。 Cas サンドボックス内で互換性を確保するための明示的な手順をライブラリ開発者が行っていない限り、CA に依存するアプリケーションは、最新のライブラリがこれらの環境内で正しく動作することを期待する必要はありません。  
