...
次の例は、匿名レコードの構文を示しています。 として区切られた項目 `[item]` は省略可能です。
...
匿名レコードは、インスタンス化の前に宣言する必要のない F # のレコード型として考えるのが最適です。
...
次の例では、前の例のを、 `printCircleStats` 匿名レコードを入力として受け取る関数を使用して展開します。
...
`printCircleStats`入力型と同じ "shape" を持たない匿名レコード型を使用してを呼び出すと、コンパイルに失敗します。
...
匿名レコードは、省略可能なキーワードを使用して構造体として定義することもでき `struct` ます。 次の例では、構造体の匿名レコードを生成して使用することにより、前のコードを強化します。
...
構造体の匿名レコードでは、呼び出しサイトでキーワードを指定する必要がない "structness の推定" も許可され `struct` ます。 この例では、を呼び出すときにキーワードを解除し `struct` `printCircleStats` ます。
...
逆パターン- `struct` 入力型が構造体の匿名レコードでない場合にを指定すると、コンパイルに失敗します。
...
`x`との `y` 値の型が異なり、相互に互換性がありません。 これらは equatable ではなく、比較できません。 これを説明するために、同等の名前付きレコードを考えてみましょう。
...
名前付きレコードの場合と同様に、匿名レコードをシリアル化できます。 [でNewtonsoft.Js](https://www.nuget.org/packages/Newtonsoft.Json/)を使用する例を次に示します。
...
### <a name="anonymous-records-interoperate-with-c-anonymous-types"></a>匿名レコードと C# 匿名型の相互運用

[C# の匿名型](../../csharp/programming-guide/classes-and-structs/anonymous-types.md)を使用する必要がある .net API を使用することができます。 C# の匿名型は、匿名レコードを使用してと相互運用するのが簡単です。 次の例は、匿名レコードを使用して、匿名型を必要とする [LINQ](../../csharp/programming-guide/concepts/linq/index.md) オーバーロードを呼び出す方法を示しています。
...
現在、データを含む匿名レコードを定義することはできません `mutable` 。 変更可能なデータを許可する [オープン言語の推奨](https://github.com/fsharp/fslang-suggestions/issues/732) 事項があります。

### <a name="limitations-with-struct-anonymous-records"></a>構造体の匿名レコードに関する制限事項

構造体の匿名レコードをまたはとして宣言することはできません `IsByRefLike` `IsReadOnly` 。 と匿名のレコードには、の [オープン言語の推奨](https://github.com/fsharp/fslang-suggestions/issues/712) 事項があり `IsByRefLike` `IsReadOnly` ます。
