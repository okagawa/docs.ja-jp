...
単体テストを記述するときは、インフラストラクチャに対する依存関係を設けようとしないでください。 テストが低速で不安定になるため、これは統合テストで行います。 アプリケーションでこれらの依存関係を回避するには、[明示的な依存関係の原則](https://deviq.com/explicit-dependencies-principle)に従い、[依存関係の挿入](../extensions/dependency-injection.md)を使用します。 個別のプロジェクトの単体テストを統合テストと区別することもできます。 これにより、単体テスト プロジェクトとインフラストラクチャ パッケージの間に参照や依存関係がなくなります。
...
テストに同様のオブジェクトまたは状態が必要な場合は、`Setup` 属性と `Teardown` 属性を利用する (存在する場合) より、ヘルパー メソッドを優先します。
