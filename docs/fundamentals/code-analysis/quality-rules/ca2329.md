...
この規則は、逆シリアル化メソッドに渡されるか、フィールドまたはプロパティとして初期化される [Newtonsoft.Json.JsonSerializer](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_JsonSerializer.htm) インスタンスに対して、次の両方の条件が当てはまる場合に適用されます。
